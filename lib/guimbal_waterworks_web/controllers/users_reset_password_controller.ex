defmodule GuimbalWaterworksWeb.UsersResetPasswordController do
  ...
  def create(conn, %{"users" => %{"email" => email}}) do
    if users = Accounts.get_users_by_email(email) do
      Accounts.deliver_users_reset_password_instructions(
        users,
        &Routes.users_reset_password_url(conn, :edit, &1)
      )
    end
  ...
  def edit(conn, _params) do
    render(conn, "edit.html", changeset: Accounts.change_users_password(conn.assigns.users))
  end
